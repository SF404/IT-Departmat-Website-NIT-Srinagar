            ...
            <Center>
                <VStack align="center" p={6} width={{ base: '100%', md: '60%' }}>
                    ...
                    <SimpleGrid columns={[2, 3, 4]} spacing={4} w={'full'}>
                        {btech.map((semester) => (
                            <Button transition={'all 0.2s ease-in'} _hover={{ transform: 'scale(1.02)', boxShadow: '0 0 12px rgba(63,81,181,0.5)' }} display={'flex'} flexDirection={'column'} colorScheme='whiteAlpha' color={'darkblue'} aspectRatio={19 / 6} w={'full'} h={'full'} as={Link} to={`/semester/${semester.id}`} boxShadow={'0 0 6px rgba(0,0,0,0.1)'}>
                                {semester.name}
